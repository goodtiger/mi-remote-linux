...
class VoicePipeline:
    ...
    def __init__(
        ...
        if engine not in {"auto", "faster-whisper", "voxtype-paraformer"}:
        ...
        if voxtype_model_dir is None:
            data_home = Path(os.environ.get("XDG_DATA_HOME", Path.home() / ".local/share"))
            voxtype_model_dir = data_home / "voxtype/models/paraformer-zh"
        self._voxtype_model_dir = Path(voxtype_model_dir)
    ...
    def _select_engine(self) -> str:
        paraformer_ready = bool(
            self._voxtype_path
            and (self._voxtype_model_dir / "model.int8.onnx").is_file()
            and (self._voxtype_model_dir / "tokens.txt").is_file()
        )
        if self.engine == "auto":
            return "voxtype-paraformer" if paraformer_ready else "faster-whisper"
        return self.engine
    ...
    def load_model(self) -> bool:
        ...
        if self.active_engine == "voxtype-paraformer":
            if not self._voxtype_path:
                ...
                return False
            if not (self._voxtype_model_dir / "model.int8.onnx").is_file():
                logger.error("未找到 Voxtype Paraformer 模型: %s", self._voxtype_model_dir)
                return False
